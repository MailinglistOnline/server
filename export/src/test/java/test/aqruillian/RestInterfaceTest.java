...
public class RestInterfaceTest {
	...
	public static WebArchive createDeployment() {
		...
		beansXml.alternativeClass(MockSearchClient.class);
		WebArchive webArchive = ShrinkWrap
				.create(WebArchive.class, "test.war")
				.setWebXML(new File("src/main/webapp/WEB-INF/web.xml"))
				.addPackages(true, "mailinglistonline.server.export")
				.addClass(MockDbClient.class)
				.addClass(MockSearchClient.class)
				.addClass(EmailCreator.class)
				.addAsWebInfResource(
	...
	public void testGetEmailById() throws Exception {
		String id = MockDbClient.FIRST_ID;
	...
	public void testAddTags() throws Exception {
		String id = MockDbClient.FIRST_ID;
		String tag = "tag";
		String url = deploymentUrl.toString() + RESOURCE_PREFIX + "/email/tag" + "?id=" +id + "&tag=" + tag;
		ClientRequest request = new ClientRequest(url);
		request.header("Accept", MediaType.APPLICATION_JSON);
		request.post();
		
		//Check that tag was added.
		String url2 = deploymentUrl.toString() + RESOURCE_PREFIX + "/email/" + id;
	    request = new ClientRequest(url2);
	    request.header("Accept", MediaType.APPLICATION_JSON);
	    ClientResponse<String> responseObj = request.get(String.class);
	    String response = responseObj.getEntity().trim();
		Assert.assertEquals(200, responseObj.getStatus());
		Assert.assertTrue(response.contains("\"from\":\"test\""));
		Assert.assertTrue(response.contains("\"tags\""));
	}
